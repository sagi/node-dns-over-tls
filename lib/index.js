...
const checkDone = (response, packetLength, socket, resolve) => {
...
const query = (...args) => new Promise((resolve, reject) => {
    ...
    const socket = tls.connect(options, () => socket.write(buf));
    socket.on('data', (data) => {
        if (response == null) {
...
const argsOrder = (args) => {
    if (isObject(args[0])) {
        const options = args[0];
        const { host, servername, name, klass = 'IN', type = 'A', port = 853, } = options;
        ...
        throw new Error('Either an options object, a tuple of host, servername, name are valid inputs or one domain name');
    }
};
/*
(async () => {
